...
> 快照截至 **2026-08-25 收盘**；每日 17:30 由 GitHub Actions 自动更新 `data/last_signal.json`（完整门控与指标明细），README 内快照随维护提交刷新。历史信号流水见 `data/signal_log.csv`。
...
| 明日目标仓位 | 0% |
| 动作 | 空仓等待 |
| 门控明细 | close>MA3 ❌（9.51 < 9.84）、MA3>MA20 ✅、RSI<80 ✅、+DI>-DI ✅、医药ETF>MA20 ❌（0.383 < 0.390）、创业板>MA30 ❌（3397.52 < 3521.97）、换手率0.8%~10% ❌（12.12%） |

数据完整性：贝瑞基因/医药ETF/医疗ETF/创业板/上证/深成指已更新至 2026-08-25 收盘（东财前复权）；基因测序板块指数更新至 2026-08-24（板块指数日线滞后一个交易日属数据源常态，非门控默认项）。8/25 信号为空仓：个股收盘跌破 MA3、医药ETF 跌破 MA20、创业板仍在 MA30 下方、换手 12.12% 超出 0.8%~10% 区间上限，均为真实数据不满足。
